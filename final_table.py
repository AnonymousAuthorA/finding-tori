...
def check_k_fold_rando_forest(embeddings, labels, k=5, num_train_ratio=0.75):
...
def get_args():
  ...
  parser.add_argument('--wandb_dir', type=str, default='pre_trained_models')
  parser.add_argument('--use_histogram', action='store_true')
  parser.add_argument('--resolution', type=float, default=0.2)


  args = parser.parse_args()
  # check either ckpt_path or code is given
  if args.ckpt_path is None and args.code is None:
    raise ValueError('Either ckpt_path or code should be given!')
...
def get_embedding_from_model(ckpt_path):
  ...
  yaml_path = ckpt_path.parent.parent / 'config.yaml'
  ...
  tori_set = ToriDataset(meta, config.contour_dir, frame_rate=config.train.frame_rate, min_length=config.train.min_length, slice_len=config.train.slice_len, return_entire=True)
...
def main():
  ...
  if args.use_histogram:
    ...
    if args.code is not None:
      ...
      ckpt_path = get_best_ckpt_path(wandb_dir, args.code)
    else:
  ...
  is_not_speech = (labels != tori_set.class2idx['speech']).float()
  ...
  ndcg_per_label = [ndcg_per_label[i] / num_sample_per_label[i] for i in range(4)]
  class_names_without_others = [x for x in tori_set.class_names if x != 'speech']
  print(f"Label names are {class_names_without_others}")
  print(f"Mean NDCG (without 'others'): {total_ndcg / len(top_k_indices_exc_speech)}, NDCG per label: {ndcg_per_label}")


  k_mean, k_std, wrong_samples = check_k_fold_rando_forest(embeddings, labels, k=10)
  print(f"Random forest accuracy: {k_mean} +- {k_std}")

  k_mean, k_std, wrong_samples = check_k_fold_rando_forest(emb_exc_speech, labels_exc_speech, k=10)
